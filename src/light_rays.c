...
/*   Updated: 2019/04/25 00:33:45 by gguichar         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static t_color	get_specular_color(t_ray_object *light, t_ray_object *object
		, double shine_factor)
{
	double	shine_f;
	...
	return (color_scalar(light->color, shine_f));
}
		...
		, t_ray_inf *ray_inf, t_ray_inf *light_ray)
{
	...
	color = (t_color){.0f, .0f, .0f};
	if (light->type == RAYOBJ_AMBIENTLIGHT)
	{
		color = color_mul(ray_inf->object->color
	...
	ray_inf->color = color_add(ray_inf->color, color);
}
