...
/*   Updated: 2019/05/04 20:18:04 by gguichar         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static t_vec3d	get_triangle_normal_for_mesh(t_vec3d origin, t_vec3d direction
		, t_vec3d *vertices, t_vec3d *normals)
{
	...
	tmp[4] = vec3d_cross(tmp[3], tmp[0]);
	var[3] = var[1] * vec3d_dot(direction, tmp[4]);
	normal = vec3d_add(vec3d_scalar(normals[0], var[2])
			, vec3d_add(vec3d_scalar(normals[1], var[3])
				, vec3d_scalar(normals[2], 1 - var[2] - var[3])));
	return (vec3d_unit(normal));
}

static int		hit_trianglemesh_depth(t_mesh_tree *tree, t_ray_hit *hit
		, double *min_dist, t_vec3d *normal)
{
	size_t		idx;
	double		dist;
	t_triangle	*triangle;
	...
	if (tree != NULL && hit_boundingbox(tree->bbox_min, tree->bbox_max, hit))
	{
		if (tree->left != NULL || tree->right != NULL)
			return (hit_trianglemesh_depth(tree->left, hit, min_dist, normal)
					|| hit_trianglemesh_depth(tree->right, hit, min_dist, normal));
		else
		{
			idx = 0;
			while (idx < tree->triangles.size)
			{
				...
				{
					*normal = get_triangle_normal_for_mesh(hit->origin
							, hit->direction, triangle->vertices
							, triangle->normals);
					*min_dist = dist;
				}
				idx++;
...
void			hit_trianglemesh(t_ray_object *object, t_ray_hit *hit)
{
	...
	min_dist = INFINITY;
	hit_trianglemesh_depth(&object->mesh_tree, hit, &min_dist, &normal);
